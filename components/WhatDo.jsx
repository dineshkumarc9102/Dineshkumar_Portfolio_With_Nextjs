import React from 'react'
import { motion } from "motion/react"
import { services } from '@/assets/assets'
import dynamic from "next/dynamic";

const OrbitSkills = dynamic(() => import("./OrbitSkills"), {
  ssr: false,
});

const ParticleVoid = dynamic(() => import("./ParticleVoid"), {
  ssr: false,
});

const WhatDo = () => {
    return (
        <motion.div
            id='whatdo'
            className='w-full px-5 sm:px-8 md:px-[12%] py-12 scroll-mt-20 overflow-hidden'
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
        >
            {/* Heading */}
            <motion.h2
                className='text-center text-3xl sm:text-5xl font-Ovo'
                initial={{ y: -20, opacity: 0 }}
                whileInView={{ y: 0, opacity: 1 }}
                viewport={{ once: true }}
                transition={{ delay: 0.2, duration: 0.5 }}
            >
                What I Do
            </motion.h2>

            <p className="text-center max-w-2xl mx-auto mt-5 mb-12 text-gray-600 dark:text-white/70 font-Ovo">
                Turning ideas into impactful digital solutions through technology, analytics, and innovation.
            </p>

            {/* Cards Grid */}
            <motion.div
                className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 my-12'
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: 0.4, duration: 0.6 }}
            >
                {services.map(({ icon: Icon, title, description }, index) => (
                    <div
                        key={index}
                        className="group relative rounded-3xl p-5 sm:p-6 md:p-7 bg-white/50 dark:bg-white/5
                                   border border-gray-200 dark:border-purple-500/20
                                   shadow-lg hover:shadow-2xl
                                   hover:border-purple-400/50 dark:hover:border-purple-500/40
                                   hover:bg-lightHover dark:hover:bg-darkHover
                                   transition-all duration-300 transform group hover:-translate-y-2 overflow-hidden"
                    >
                        {/* Number */}
                        <span className="absolute top-4 right-4 text-3xl md:text-4xl font-bold text-black/10 dark:text-white/10 select-none">
                            0{index + 1}
                        </span>

                        {/* Icon */}
                        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-purple-500/10 border border-purple-500/20 flex items-center justify-center mb-5 group-hover:bg-purple-500/20 transition-colors duration-300">
                            <Icon className="w-6 h-6 sm:w-7 sm:h-7 text-purple-400" />
                        </div>

                        {/* Title */}
                        <h3 className="text-base sm:text-lg font-semibold mb-3 text-gray-900 dark:text-white">
                            {title}
                        </h3>

                        {/* Description */}
                        <p className="text-sm sm:text-base leading-6 sm:leading-7 text-gray-600 dark:text-white/70">
                            {description}
                        </p>
                    </div>
                ))}
            </motion.div>

            {/* Skills Section */}
            <div className="text-center mt-24">
                <motion.h3
                    className="text-2xl sm:text-3xl font-Ovo mb-2"
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.6 }}
                >
                    Skills & Tools
                </motion.h3>

                {/* ORBIT WRAPPER (Unified Animations to Prevent Canvas Stuttering) */}
                <motion.div
                    className="relative flex justify-center items-center h-[500px] sm:h-[800px] md:h-[850px] overflow-hidden rounded-3xl"
                    initial={{ opacity: 0, scale: 0.95 }}
                    whileInView={{ opacity: 1, scale: 1 }}
                    viewport={{ once: true, amount: 0.1 }}
                    transition={{ duration: 0.8, ease: "easeOut" }}
                >
                    {/* Background Canvas Effect */}
                    <div className="absolute inset-0 z-0 pointer-events-none opacity-60 dark:opacity-100">
                        <ParticleVoid
                            style={{ width: "100%", height: "100%" }}
                            x={50}
                            y={50}
                            radius={40}
                            density={25}
                            gap={45}
                            particleSize={3}
                            colors={["#000000", "#ffffff"]} // Added hints of purple matching theme stars
                            direction="inside"
                            speed={1.5}
                        />
                    </div>

                    {/* Orbit Core Graphic */}
                    <div className="relative z-10 w-full h-full flex items-center justify-center">
                        <OrbitSkills />
                    </div>
                </motion.div>
            </div>
        </motion.div>
    )
}

export default WhatDo
